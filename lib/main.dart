import'package:flutter/material.dart';
import 'package:kura_health/blogger.dart';
import 'pharamacy.dart';
import 'form_page.dart';
import 'hospitals.dart';
import 'SINWN.dart';
import 'specialist.dart';
import 'caretaker.dart';
import 'alarm_page.dart';
void main(){
  runApp(const  AlarmPage());
}
class KuraApp extends StatefulWidget {
  const KuraApp({Key? key}) : super(key: key);
  @override
  State<KuraApp> createState() => _KuraAppState();
}
class _KuraAppState extends State<KuraApp> {
  // final _auth = FirebaseAuth.instance;
  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
  List<Book> books = allBooks;
  wide({place, text}) {
    return Column(
      children: [
        CircleAvatar(
          minRadius: 25,
          backgroundColor: const Color.fromRGBO(64, 188, 140, 0.4),
          child: CircleAvatar(
            backgroundColor: const Color.fromRGBO(64, 188, 140, 1),
            minRadius: 18,
            child: place,
          ),
        ),
        text
      ],
    );
  }
  void _openDrawer() {
    _scaffoldKey.currentState!.openDrawer();
  }

  void _closeDrawer() {
    Navigator.of(context).pop();
  }
  // void initState() {
  //   super.initState();
  //   getCurrentUser();
  // }
  // void getCurrentUser() async{
  //   try{
  //     final user = await _auth.currentUser!.email;
  //   }
  //   catch(e){print(e);}
  // }
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      darkTheme: ThemeData(primaryColor: const Color.fromRGBO(64, 188, 140, 1)),
      debugShowCheckedModeBanner: false,
      home: Scaffold(
        key: _scaffoldKey,
        appBar: AppBar(
          backgroundColor: const Color.fromRGBO(64, 188, 140, 1),
          elevation: 0,
          leading: IconButton(
          icon: const Icon(Icons.menu, size: 30), onPressed: _openDrawer,
          ),
          //IconButton(
           // icon: const Icon(Icons.menu, size: 30), onPressed: () {
            //Navigator.push(context, MaterialPageRoute(builder: (context) => const Splash()));
         // },
          //),
          actions: const [
            CircleAvatar(
              radius: 50.0,
              backgroundImage: AssetImage('profile.jpg'),
            )
          ],
        ),
        drawer: Drawer(
          width: 400,
          backgroundColor: Colors.white,
          elevation: 2.0,
          child: ListView(
            children: [
              Padding(padding: const EdgeInsets.all(34,),
                child: Column(
                  children: [
                    ListTile(leading: const CircleAvatar(backgroundColor: Colors.cyan,), trailing: Column(children: const [Text('Hans Koehler', style: TextStyle(fontWeight: FontWeight.bold),)],)),
                    ListTile(
                      leading: const Icon(Icons.person_outline_outlined),
                      trailing: const Text('Profile'),
                      onTap: _closeDrawer,
                    ),
                    ListTile(
                      leading: const Icon(Icons.person_outline_outlined),
                      trailing: const Text('Profile'),
                      onTap: _closeDrawer,
                    ),
                    ListTile(
                      leading: const Icon(Icons.person_outline_outlined),
                      trailing: const Text('Profile'),
                      onTap: _closeDrawer,
                    ),
                    ListTile(
                      leading: const Icon(Icons.person_outline_outlined),
                      trailing: const Text('Profile'),
                      onTap: _closeDrawer,
                    ),
                  ],
                ),
              ),
            ],
          ),
        ),
        drawerEnableOpenDragGesture: false,
        body: ListView(
          children: [
            Container(
              height: 9999999,
              color: const Color.fromRGBO(64, 188, 140, 1),
              child: Column(
                children: [
                  Padding(
                    padding: const EdgeInsets.fromLTRB(0, 35, 35, 35),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Welcome name',
                          style: TextStyle(fontSize: 30,
                              color: Colors.white,
                              fontWeight: FontWeight.w100),),
                        const SizedBox(
                          height: 30,
                        ),
                        Container(
                          height: 50,
                          width: 350.0,
                          decoration: const BoxDecoration(
                            borderRadius: BorderRadius.all(
                                Radius.circular(15.0)),
                            color: Colors.white,
                          ),
                          child: Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: TextField(
                              decoration: const InputDecoration(
                                hintText: 'Type your search',
                                hintStyle: TextStyle(
                                  color: Colors.black12,
                                ),
                                icon: Icon(Icons.search),
                              ),
                              onChanged: searchBook,
                            ),
                          ),
                        ),
                      ],
                    ),
                  ),
                  Container(
                    height: 999700,
                    decoration: const BoxDecoration(
                      color: Colors.white,
                      borderRadius: BorderRadius.only(
                          topLeft: Radius.elliptical(150, 30),
                          topRight: Radius.elliptical(200, 30)),
                    ),
                    child: Padding(
                      padding: const EdgeInsets.fromLTRB(0, 40, 0, 0),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text("Services", style: TextStyle(
                              fontSize: 20)),
                          const SizedBox(
                            height: 20,
                          ),
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                            children: [
                              wide(place: IconButton(
                          icon: const Icon(Icons.medical_services, color: Colors.white,), onPressed: () {
                          Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const Specialist()));
                    },
                    ),
                                  text: const Text("Specialist")),
                              wide(place: IconButton(
                                icon: const Icon(Icons.medication, color: Colors.white,), onPressed: () {
                                Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const PharmacyApp()));
                              },
                              ),
                                  text: const Text("Pharmacy")),
                              wide(place: IconButton(icon: const Icon(Icons.calendar_month_sharp, color: Colors.white,),
                                  onPressed: () {Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const AlarmPage()));}),
                                  text: const Text("Reminder")),
                              wide(place: IconButton(
                                icon: const Icon(Icons.person, color: Colors.white,), onPressed: () {
                                Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const Caretaker()));
                              },
                              ), text: const Text("Caretaker")),
                              wide(place: GestureDetector(
                                child: IconButton(
                                  icon: const Icon(Icons.local_hospital, color: Colors.white,), onPressed: () {
                                  Navigator.push(context, MaterialPageRoute(builder: (context) => HospitalPage()));
                                },
                                ),
                              ), text: const Text("Hospital")),
                            ],
                          ),
                          const SizedBox(height: 35,),
                          Container(
                            color: const Color.fromRGBO(64, 188, 140, 0.3),
                            height: 10,
                          ),
                          const SizedBox(height: 35,),
                          Expanded(
                            child: SizedBox(
                              height: 1509,
                              child: ListView.builder(itemCount: books.length,itemBuilder: (context, index){
                                final book = books[index];
                                return Padding(
                                  padding: const EdgeInsets.all(8.0),
                                  child: ListTile(
                                    title: Text(book.title), trailing: SizedBox(height: 300, width: 250, child: Image.network(book.urlImage, fit: BoxFit.cover)), ),
                                );
                              }, ),
                            ),
                          ),
                        ],
                      ),
                    ),
                  ),
                ],
              ),
            ),
          ],
        ),
      ),
    );
  }
  void searchBook(String query) {
    final suggestions = allBooks.where((book) {
      final bookTitle = book.title.toLowerCase();
      final input = query.toLowerCase();
      return bookTitle.contains(input);
    }).toList();
    setState(() => books = suggestions);
  }
}
class Book{
  final String title;
  final String urlImage;
  final String blogLink;
  const Book({
    required this.title, required this.urlImage, required this.blogLink,
  });
}
var allBooks = [
 const Book(
      title: 'false Bla la', urlImage: 'data:image/jpeg;base64,', blogLink: 'https://unsplash.com/s/photos/pi'
  ),
 const Book(
      title: 'true Bla la', urlImage: 'https://unsplash.com/s/photos/pi1', blogLink: 'https://unsplash.com/s/photos/pi'
  ),
  const Book(
      title: 'tout Bla la', urlImage: 'https://unsplash.com/s/photos/pi1', blogLink: 'https://unsplash.com/s/photos/pi'
  ),
];
// class CustomHeader{
//   final String title;
//   final String hintText;
//   cheader({required padding}) {
//     return Container(
//       color: const Color.fromRGBO(64, 188, 140, 1),
//     );
// }
// }
// Header({required padding}) {
//     return Container(
//       color: const Color.fromRGBO(64, 188, 140, 1),
//     );
//}